...
<?php
	...
	echo '</pre>';
?>
	<h1>Summary of {{ $company->company_name }}</h1>
	<form method="post" enctype="multipart/form-data">
		<div class="form-group">
			<label>Analyst</label>
			<input type="text" name="data[posted_by]" required value="{{ request()->old('data.posted_by') }}" class="form-control">
		...
		<div class="form-group">
			...
			<input type="text" name="data[analyst_post]" required value="{{ request()->old('data.analyst_post') }}" class="form-control">
			@if($errors->has('analyst_post'))
				<span class="error-block">
					@foreach($errors->get('analyst_post') as $e)
						<p>{{ $e }}</p>
					@endforeach
				</span>
			@endif
		...
		<div class="form-group">
			<label>Analyst Image</label>
			<div class="row">
				<div class="col-md-6 col-sm-12">
					<select required name="data[asset_type]">
		...
		<a href="{{ route('admin-company-summary-list-get', $company->id) }}" class="btn btn-info">Cancel</a>
	</form>

	<div id="hidden-block" style="display:none">
		<div class="form-group" id="image">
			
			<input type="file" name="data[asset]" required>
		...
		<div class="form-group" id="video">
			<input type="text" name="data[asset]" class="form-control" required>
			@if($errors->has('data.asset'))
				<span class="error-block">
					@foreach($errors->get('asset') as $e)
						<p>{{ $e }}</p>
	...
	<input type="hidden" id="prabal-ajax-upload-image-directory" value="company-summary">
	<input type="hidden" id="prabal-ajax-upload-image-asset-type" value="company-summary">
