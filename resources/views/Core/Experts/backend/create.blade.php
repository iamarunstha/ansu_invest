...
?>
	<form method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			...
			<input type="number" name="data[ordering]" class="form-control" value="{{ request()->old('data.ordering')  }}" step=1>
			...
			<div class="row related-companies">
				@foreach($companies as $c)
					<div class="col-sm-12 col-md-4" shortcode = "{{$c->short_code}}">
						<input type="checkbox" name="company[company_id][]" value="{{ $c->id }}">{{ $c->company_name }}
					</div>
				@endforeach
			</div>
		</div>
