...
?>
	<form method="post" enctype="multipart/form-data">
		...
		<div class="form-group">
			<label>Is this News or Report?</label>
			<select name="data[news_type]">
				<option value="news" @if($data->news_type == 'news')selected @endif >News</option>
				<option value="report" @if($data->news_type == 'report')selected @endif>Report</option>
			</select>
		</div>

		<div class="form-group">
			<label>Tags</label>
			<input type="text" name="tags[tags]" class="form-control" value="{{ $tags }}">
			@if($errors->has('tags'))
				<span class="error-block">
		...
		<div class="form-group">
			<label>Related Companies</label><br>
			<input id="myInput" type="text" placeholder="Search...">
			<div class="row related-companies">
				@foreach($companies as $c)
					<div class="col-sm-12 col-md-4" shortcode = "{{$c->short_code}}">
						<input type="checkbox" name="company[company_id][]" value="{{ $c->id }}" @if(in_array($c->id, $related_companies)) checked @endif>{{ $c->company_name }}
					</div>
				@endforeach
			</div>
		...
		<a href="{{ route('admin-news-list-get', $type[0]) }}" class="btn btn-info">Cancel</a>
	</form>

	<div id="hidden-block" style="display:none">
		<div class="form-group" id="image">
			@if($data->asset_type == 'image') 
				<img src="{{ route('get-image-asset-type-filename', ['news', $data->asset]) }}" height="150px">
			@endif
	...
	<script>
		$(document).ready(function(){
  			$("#myInput").on("keyup", function() {
    			...
    			$("div.related-companies div").filter(function() {
      				$(this).toggle($(this).text().toLowerCase().indexOf(value) > -1 || $(this).attr("shortcode").toLowerCase().indexOf(value) > -1)
    			});
  			});
		});
	</script>
