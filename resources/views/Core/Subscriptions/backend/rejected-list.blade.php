...
<!-- Page Heading -->
  	<h1 class="h3 mb-4 text-gray-800">Subscription Rejected Requests</h1>
	...
	@foreach($data as $index => $d) 
		...
		<form method="post" action="{{ route('admin-subscripton-requests-delete-post', $d->id) }}" id="delete-{{ $d->id }}" class="prabal-confirm">
			{{ csrf_field() }}
		</form>
	@endforeach
@stop
