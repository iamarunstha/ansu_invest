...
@if(Auth::user()->group_id == 1)
	...
	<form method="post" action="{{route('admin-register-permissions-post')}}">
		{{ csrf_field() }}
		<input type="hidden" name="module" value="NEPSE Index">
...
@section('content')
  	...
  	<!-- Page Heading -->
  	<h1 class="h3 mb-4 text-gray-800">Upload Nepse</h1>
		
  	<form method="post" enctype="multipart/form-data">
  		<div class="col-md-4">
			<div class="form-group">
				<label>Download</label>
				<button type="button" href="{{ route('donwload-upper-nepse-get') }}" class="btn btn-info form-control" id="download-sample">
					Download Sample
				</button>
			</div>
		</div>
